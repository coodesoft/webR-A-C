 ...
 * @since 1.0
 */
class PEmailLogRoute extends PLogRoute
{
    /**
    ...
    {
        $message='<html><body><style>p{margin: 0;}</style>';
        $message.="<p>--------------------------------------------------------</p>";
        $message.="<p><strong>USER: </strong>" . Yii::app()->user->name."</p>";
        $message.="<p>--------------------------------------------------------</p>";
        foreach($logs as $log)
            $message.=$this->formatLogMessage(str_replace("\n", "<p>--------------------------------------------------------</p>", $log[0]),$log[1],$log[2],$log[3]);
