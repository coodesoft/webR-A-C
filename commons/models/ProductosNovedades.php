...
class ProductosNovedades extends CActiveRecord
{
    ...
    public static function seteoNovedad($producto_novedad = null, $categoria_id, $producto_id)
    {
        $sql = 'SELECT * FROM
                        productos_novedades WHERE categoria_id = '.$categoria_id.'
                        AND producto_id = '.$producto_id;
        $nov = ProductosNovedades::model()->findBySql($sql);
        if ($producto_novedad && $nov === null) {
            $pn = new ProductosNovedades();
            $pn->categoria_id = $categoria_id;
            $pn->producto_id = $producto_id;
            $pn->save();
        } else if (!$producto_novedad && $nov !== null) {
            ProductosNovedades::model()->deleteAllByAttributes(
                array(
    ...
    public static function isNovedad($producto_id, $categoria_id)
    {
        $sql = "SELECT * FROM productos_novedades WHERE producto_id = " . $producto_id . "
                AND categoria_id = " . $categoria_id;
        $novedades = ProductosNovedades::model()->findAllBySql($sql);

        return count($novedades) ? 1 : 0;
    }
}
